...
at step~18 (where Path~B's loss first drops below Path~A's hop loss $\num{4.73e-9}$
to $\num{4.34e-9}$, preceded by $\num{5.84e-9}$ at step~17), resetting momentum yields a post-restart peak violation of
$1.74\%$, confirming that travel distance across
the ill-conditioned landscape is the dominant driver of realizability excursions,
while continuous momentum contributes an incremental $+0.26\%$ amplification ($1.74\% \to 2.00\%$).
Comparing Adam with and without first-moment momentum ($\beta_1 = 0$) at a matched
convergence rate ($76$ steps to $L < \num{1.4e-9}$) isolates a $1.27\times$
momentum amplification ($1.65\%$ vs.\ $1.30\%$), while a systematic step-size sweep
shows that traversal speed strongly modulates excursion magnitude. Convergent plain
gradient descent traverses unrealizable states at $6.25\%$ (Path~B) and $6.16\%$
(Path~A). Furthermore, we prove that per-coordinate Adam on a unit-normalized basis
preserves exact scale invariance to machine precision (relative loss discrepancy
$< \num{3e-15}$, parameter difference $< \num{3e-17}$).

In this convex setting, direct warm-starting accelerates convergence (reaching tolerance in $59$ steps vs.\ $101$ for cold start), but incurs a higher peak violation ($2.00\%$ vs.\ $1.22\%$). Staged warm-starting (Path~A) is both slower ($123$ total steps) and accumulates higher deployed violation ($5.90\%$), directly inheriting the intermediate scaffold's boundary excursion. Un-staged cold start is therefore optimal for realizability preservation, while direct warm-starting minimizes step count. We formalize this configuration space as a \emph{Stratified Design Atlas} and delineate the necessary conditions---non-convex losses, coupled Navier--Stokes evaluation, and non-nested basis edits---under which geometric transport and path-dependence become non-trivial.
\end{abstract}
...
\begin{enumerate}
  ...
  \item We analyze the dynamics of staged calibration ($\mathcal{M}_0 \to
    ...
    $5.64\%$ violation upon deployment (peaking at $5.90\%$). Direct warm-starting
    achieves the fastest convergence to tolerance ($59$ steps, saving $42$ steps
    over cold start), but incurs a $2.00\%$ peak violation. Cold-starting achieves
    the lowest peak violation ($1.22\%$) throughout descent, at the cost of
    moderate latency ($101$ steps), while staged warm-starting is the slowest
    protocol ($123$ steps) with the worst realizability profile.
  \item Through a suite of pre-registered controls (matched-loss restart at
    $\num{4.34e-9}$, Adam $\beta_1 = 0$ at matched convergence rate of $76$ steps,
    step-size sweeps, and per-coordinate normalization), we isolate the mechanisms
    governing transient excursions: parameter travel distance across the ill-conditioned
    Gram landscape dominates ($1.74\%$ vs.\ $2.00\%$), while momentum contributes
    an amplification factor of $1.27\times$ ($1.65\%$ vs.\ $1.30\%$) at matched speed.
    Under unit normalization, per-coordinate Adam restores exact scale invariance to
    machine precision.
\end{enumerate}
...
\caption{Audited traversal telemetry across protocols ($N = 220$, $\lambda = 150$).
  ...
  Path~A = 126, Path~B = 89, Path~C = 111. Secondary vorticity forcing correlation metrics
  ($\rho$ and reversed fraction) were tracked along the warm-started and staged trajectories
  (Paths~A and~B); Path~C focuses on the realizability-recovery trajectory and forcing columns are marked ---.}\label{tab:telemetry}
\begin{ruledtabular}
\begin{tabular}{llccccc}
  ...
  \multirow{9}{*}{Path~A (Staged)}
    ...
    & Tol.\ reached ($L < \num{1.4e-9}$) & 123 & $\num{1.39e-9}$ & $1.30\%$ & $+0.994$ & $1.78\%$ \\
    & $1\%$ above final ($L \le \num{1.355e-9}$) & 126 & $\num{1.35e-9}$ & $1.13\%$ & $+0.998$ & $1.65\%$ \\
    & Final & 219 & $\num{1.34e-9}$ & $1.04\%$ & $+0.999$ & $1.65\%$ \\
  \midrule
  \multirow{8}{*}{Path~B (Direct)}
    & Rapid descent & 1 & $\num{1.11e-7}$ & $0.00\%$ & $+0.941$ & $10.11\%$ \\
    & Excursion onset & 20 & $\num{2.78e-9}$ & $1.56\%$ & $+0.955$ & $2.91\%$ \\
    & Excursion trough & 21 & $\num{2.62e-9}$ & $1.65\%$ & $+0.957$ & $2.82\%$ \\
    & Peak violation & 25 & $\num{3.85e-9}$ & $2.00\%$ & $+0.965$ & $2.73\%$ \\
    & Rebound peak ($1.68\times$) & 28 & $\num{4.40e-9}$ & $2.00\%$ & $+0.974$ & $2.65\%$ \\
    & Tol.\ reached ($L < \num{1.4e-9}$) & 59 & $\num{1.40e-9}$ & $1.22\%$ & $+0.998$ & $1.78\%$ \\
    & $1\%$ above final ($L \le \num{1.355e-9}$) & 89 & $\num{1.35e-9}$ & $1.13\%$ & $+0.998$ & $1.74\%$ \\
...
While direct warm-starting (Path~B) provides a $1.71\times$ convergence-speed advantage
over cold start ($59$ vs.\ $101$ steps, saving $42$ steps), staged warm-starting
(Path~A) is the slowest protocol ($123$ steps), demonstrating that intermediate
scaffolding imposes a $22$-step latency penalty relative to cold start while
inheriting boundary excursions upon deployment.
...
\footnotetext[3]{Evaluated at the $N = 220$ benchmark horizon ($L = \num{1.4360e-9}$). Extended optimization up to $450$ steps reaches $L = \num{1.4324e-9}$, exhibiting a slow monotone crawl along the shallow Gram direction above the baseline target ($\num{1.3412e-9}$). At matched convergence rate ($76$ steps to $L < \num{1.4e-9}$), full Adam ($\alpha = \num{1.37e-3}$) incurs $1.65\%$ peak violation vs.\ $1.30\%$ for $\beta_1 = 0$, isolating a $1.27\times$ momentum amplification factor.}
\end{table}
...
\begin{enumerate}
  \item \textit{The role of step size and travel distance.}
    ...
    and produces a peak violation of $6.25\%$ ($6.16\%$ for Path~A). Transient violations occur
    whenever an optimizer travels through parameter space at a rate sufficient
    to converge.
  \item \textit{Momentum vs.\ effective traversal speed.}
    ...
    to $1.05\times$ at nominal step size $\alpha = \num{2e-3}$.
    However, comparing $\beta_1 = 0.9$ against $\beta_1 = 0$ at fixed nominal $\alpha$
    partially conflates momentum with effective traversal speed: with momentum, the
    effective update scale is inflated by $(1-\beta_1)^{-1} \approx 10$.
    Indeed, the step-size Pareto sweep (Sec.~\ref{sec:pareto}) shows that full Adam
    with $\beta_1 = 0.9$ at reduced learning rates ($\alpha \approx \num{4.8e-4}$)
    \emph{also} achieves $1.30\%$ peak violation.
    To rigorously isolate the causal contribution of first-moment momentum, we compare
    full Adam and Adam without momentum at \emph{matched convergence rate}:
    both reach tolerance ($L < \num{1.4e-9}$) in exactly $76$ steps (full Adam at
    $\alpha = \num{1.37e-3}$ and Adam with $\beta_1 = 0$ at $\alpha = \num{2e-3}$).
    At matched speed, full Adam incurs a peak violation of $1.65\%$, compared to
    $1.30\%$ for $\beta_1 = 0$. First-moment momentum thus accounts for an
    amplification factor of $1.65\% / 1.30\% \approx 1.27\times$ in peak boundary
    excursion, rather than the un-matched nominal ratio of $1.54\times$ ($2.00\% / 1.30\%$).\footnote{In our pre-registered investigation, extended optimization of the $\beta_1 = 0$ arm up to $450$ steps reached a final loss of $\num{1.4324e-9}$ ($1.068\times$ above the baseline target of $\num{1.3412e-9}$), exhibiting a slow monotone crawl along the shallow Gram direction. The matched-loss condition ($L \le \num{1.3412e-9}$) was therefore not achieved by the zero-momentum arm within practical horizons, reinforcing the necessity of comparing optimizers along the matched-convergence-rate Pareto frontier rather than asymptotic loss endpoints.}
...
\begin{itemize}
  ...
  \item \textit{Matched-loss Path~B restart at step~18.} To eliminate the
    ...
    first drops below Path~A's hop loss ($\num{4.73e-9}$) to $\num{4.34e-9}$
    (with preceding iterate at $\num{5.84e-9}$).
    Following this matched restart, Path~B exhibits a post-restart peak violation
    of $1.74\%$ at step~21 (climbing from $0.95\%$ at step~17), while the loss
    continues its monotonic descent toward tolerance without an additional rebound.
\end{itemize}
Resetting momentum at matched loss reduces the peak excursion only modestly from
$2.00\%$ to $1.74\%$, confirming that travel distance across the ill-conditioned
parameter landscape dominates the excursion, while continuous mid-descent momentum
provides an incremental amplification of $+0.26\%$.
Furthermore, inspection of the per-step violation profile reveals that violations were
already climbing before the restart ($0.95\%$ at step~17, rising to $1.22\%$, $1.48\%$,
$1.65\%$, and $1.74\%$ across steps~18--21), demonstrating that the restart merely
interrupted an in-progress boundary traversal rather than preventing one.

\subsection{Step-Size Pareto Frontier}\label{sec:pareto}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/pareto_stepsize_sweep.png}
\caption{Step-size Pareto frontier of peak realizability violation versus steps to convergence tolerance ($L < \num{1.4e-9}$). For Plain GD, increasing $\alpha$ accelerates convergence down to $96$ steps at $\alpha = 5179$ while inflating peak violation to $6.25\%$; beyond this point, $\alpha = 10^4$ encounters a stability knee where convergence fails ($L_{\mathrm{final}} = \num{2.55e-9}$). For Adam, $\alpha = \num{6.58e-3}$ achieves the minimum step count ($46$ steps, $2.86\%$ violation); higher rates ($\alpha = \num{3.16e-2}$) degrade efficiency ($57$ steps) while driving peak violation to $4.86\%$. At matched convergence rate ($76$ steps), full Adam ($\alpha = \num{1.37e-3}$) incurs $1.65\%$ peak violation, compared to $1.30\%$ for Adam without momentum ($\beta_1 = 0$, $\alpha = \num{2e-3}$).}\label{fig:pareto}
\end{figure}
...
Figure~\ref{fig:pareto} depicts the resulting Pareto frontier of peak realizability
violation versus steps required to reach convergence tolerance ($L < \num{1.4e-9}$).

For Plain GD, increasing the step size $\alpha$ from $1.0$ to $5179.5$ accelerates
convergence from $> 220$ steps down to $96$ steps, but systematically inflates the
peak realizability violation from $0.00\%$ (stationary) up to $6.25\%$.
At $\alpha = 10^4$, Plain GD encounters a stability knee: convergence fails within
$220$ steps ($L_{\mathrm{final}} = \num{2.55e-9}$), producing higher final error
alongside $6.51\%$ violation.
For Adam, varying the base learning rate $\alpha$ across orders of magnitude from
$\num{1e-4}$ to $\num{3.16e-2}$ reveals a convex Pareto trade-off with its own
stability inflection. Small step sizes achieve low peak violation ($1.30\%$ at
$\alpha = \num{4.81e-4}$) at the cost of slow convergence ($171$ steps).
The most step-efficient regime occurs at $\alpha = \num{6.58e-3}$, reaching tolerance
in $46$ steps with a moderate peak violation of $2.86\%$. Aggressive rates beyond
this point degrade convergence efficiency while escalating boundary excursions
($57$ steps and $4.86\%$ peak violation at $\alpha = \num{3.16e-2}$).

Comparing optimizers along this frontier illuminates the role of momentum:
at matched convergence speed ($76$ steps to tolerance), full Adam with $\beta_1 = 0.9$
($\alpha = \num{1.37e-3}$) incurs $1.65\%$ peak violation, whereas Adam with
$\beta_1 = 0$ ($\alpha = \num{2e-3}$) incurs $1.30\%$. First-moment momentum
thus contributes an isolated $1.27\times$ amplification ($1.65\% / 1.30\%$),
demonstrating that traversal speed across the ill-conditioned Gram landscape is
the primary determinant of realizability excursions, while momentum acts as a
secondary amplifier.
...
In a convex, constant-coefficient, \emph{a~priori} calibration, staging calibration across intermediate strata confers no advantage in endpoint accuracy or peak realizability violation over an un-staged cold start. Staged warm-starting (Path~A) is the slowest protocol ($123$ steps to tolerance), costing $22$ steps relative to cold start ($101$ steps), while inheriting the intermediate scaffold's boundary violations upon deployment ($5.64\%$, rising to $5.90\%$). Direct warm-starting (Path~B) achieves the fastest convergence ($59$ steps, saving $42$ steps over cold start), but incurs a higher peak violation ($2.00\%$ vs.\ $1.22\%$). Cold start ($\theta = \mathbf{0}$, Path~C) achieves the lowest peak violation in the study ($1.22\%$) and reaches the identical minimum without transient loss rebounds. Staging thus trades the speed of direct warm-starting for the worst realizability profile of all three paths. Zero-padding preserves predictions identically ($\delta_R \equiv 0$), meaning the deployed model inherits whatever boundary violations were accumulated by the scaffold.
...
\begin{enumerate}
  ...
  \item Isolated momentum and travel-distance mechanics via pre-registered controls:
    ...
    (Branch~ii occurred), confirming that parameter travel
