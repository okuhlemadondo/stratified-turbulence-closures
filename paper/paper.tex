...
to $\num{4.34e-9}$), resetting momentum yields a post-restart peak violation of
$1.74\%$ with a $3.12\times$ loss rebound, confirming that travel distance across
the ill-conditioned landscape is the dominant driver of realizability excursions.
The post-restart rebound is exacerbated by Adam's cold-update transient
($v = 0 \implies |\Delta\theta_1| = \alpha$), rather than mitigated by it.
...
Crucially, the restart worsened the loss rebound from $1.68\times$ (uninterrupted Path~B)
to $3.12\times$: the cold restart introduces the same initial $v = 0$ unconditioned
sign-step perturbation identified in Path~A's post-hop transient, temporarily kicking
the optimizer deeper into the barrier. Furthermore, inspection of the per-step
violation profile reveals that violations were already climbing before the restart
($0.95\%$ at step~17, rising to $1.22\%$, $1.48\%$, $1.65\%$, and $1.74\%$ across steps~18--21),
demonstrating that the restart truncated an in-progress boundary traversal rather than preventing one.
