...
- **Momentum Amplifies Overshoot:** Setting $\beta_1 = 0$ in Adam reduces peak violation from $2.00\%$ to $1.30\%$ at matched final loss ($1.3412 \times 10^{-9}$).
...
### 2. Decisive Restart Controls

- **Matched-Loss Path B Restart (Step 18):** When Path B is restarted at the step where its loss matches Path A's hop loss ($4.73 \times 10^{-9}$), the post-restart peak violation is **$1.74\%$** with a **$3.12\times$** loss rebound (**Branch ii occurred**). This proves that travel distance across the ill-conditioned landscape dominates the excursion, with mid-descent momentum contributing a modest $+0.26\%$ amplification ($1.74\% \to 2.00\%$).
...
### 3. Traversal Dynamics and Cold-Start Optimality

- **Path A (Staged):** Scaffold peaks at $6.08\%$. At the hop ($\delta_R \equiv 0$), the deployed model inherits $5.64\%$, rises to $5.90\%$ at step 101, descends to $0.95\%$ at step 150, and rises mildly to $1.04\%$ at step 219. Staging does not eliminate boundary violations; it inherits them.
- **Path B (Direct):** Direct jump from Stratum 0 experiences a transient peak of $2.00\%$ and a $1.68\times$ rebound before converging to $1.04\%$. Reaches tolerance ($L < 1.4 \times 10^{-9}$) in 60 steps.
- **Path C (Cold Start, $\theta = \mathbf{0}$):** Achieves the **lowest peak violation in the study ($1.22\%$)**, reaching the identical minimizer without loss rebounds. Reaches tolerance in 102 steps (faster than Path A's 124 steps).
